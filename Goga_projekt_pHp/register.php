...
print '
	...
	function randomPassword() {
		...
		$pass = '';
		for ($i = 0; $i < 8; $i++) {
			$letter = rand(0, mb_strlen($alphanumeric) - 1);
			$pass .= mb_substr($alphanumeric, $letter, 1);
		}
		return $pass;
	}
	...
	if(isset($_POST['submitData'])){
		...
		if ( $_POST['password'] != $_POST['passwordretype'] ) {
			...
			if ($row['username'] != ''){
				...
				$query  = "INSERT INTO users (user_firstname, user_lastname, user_email, user_country, user_name, user_psw, user_role)";
				$query .= " VALUES ('";
	...
	<form action="index.php?menu=7" id="contact_form" name="contact_form" method="POST">
		<center><h1>Registration form<br/>
